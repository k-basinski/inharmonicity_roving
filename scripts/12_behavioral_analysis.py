...
# do some cleaning
data.response = data.response.replace({"nie wiem": pd.NA})
data.response = pd.to_numeric(data.response)

data.condition = data.condition.replace(
    {"h": "harmonic", "i": "inharmonic", "ic": "changing"}
)
...
dt1.response = dt1.response.replace(
    {1: "One sound", 2: "Two sounds", 3: "Three or more sounds"}
)
...
# apply additional jitter for plotting
dt2['abs_error_jit'] = dt2['abs_error'] + np.random.uniform(-.3, .3, len(dt2))
...
# %%
# summarize ground truth stats
dt2r['gt'].mean().round(2)
# %%
dt2r['gt'].std().round(2)
# %%
dt2r['gt'].max().round(2)
